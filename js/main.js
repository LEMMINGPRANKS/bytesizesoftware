...
async function main() {
  ...
  function snapshot() {
    return {
      ...
      levers: Array.from(world.levers),
      pistons: Array.from(world.pistons.entries()).map(([k, v]) => [k, v]),
    };
  }
  ...
  function bootWorld(mode, seed, saved) {
    ...
    if (saved && saved.levers) {
      for (const k of saved.levers) world.levers.add(k);
      world.powerDirty = true;
    }
    if (saved && saved.pistons) {
      for (const [k, v] of saved.pistons) {
        if (v && Array.isArray(v.facing) && v.facing.length === 3) {
          world.pistons.set(k, { facing: v.facing, sticky: !!v.sticky });
        }
      }
    }
